...
class AuthTokenTest < ActiveSupport::TestCase
  test "valid auth token creation" do
    user = users(:alice)
    auth_token = AuthToken.new(user:, provider: "slack", access_token: "token123", scopes: ["chat:write"])
    assert auth_token.valid?
  end

  test "provider presence validation" do
    user = users(:alice)
    auth_token = AuthToken.new(user:, provider: "", access_token: "token123")
  ...
  test "access token presence validation" do
    user = users(:alice)
    auth_token = AuthToken.new(user:, provider: "github", access_token: "")
    assert_not auth_token.valid?
  ...
  test "uniqueness of provider per user" do
    user_a = users(:alice)
    AuthToken.create!(user: user_a, provider: "slack", access_token: "token_a", scopes: ["chat:write"])

    auth_token = AuthToken.new(user: user_a, provider: "slack", access_token: "token_b", scopes: ["chat:read"])
    assert_not auth_token.valid?
    assert auth_token.errors[:provider].any?
  end
  ...
  test "same provider allowed for different users" do
    ...
    user_b = users(:bob)
    AuthToken.create!(user: user_a, provider: "slack", access_token: "token_a", scopes: ["chat:write"])
    auth_token = AuthToken.new(user: user_b, provider: "slack", access_token: "token_b", scopes: ["chat:read"])
    assert auth_token.valid?
  end
  ...
  test "user has many auth tokens" do
    user = users(:alice)
    auth_token = AuthToken.create!(user:, provider: "slack", access_token: "token123", scopes: ["chat:write"])
  ...
  test "access token encryption" do
    ...
    auth_token = AuthToken.create!(user:, provider: "github", access_token: access_token_value, scopes: ["repo"])
  ...
  test "refresh token encryption" do
    ...
    auth_token = AuthToken.create!(user:, provider: "github", access_token: "access", refresh_token: refresh_token_value, scopes: ["repo"])
